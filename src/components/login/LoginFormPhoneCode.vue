<template>
  ...
  >
    ...
    <a-form-item :wrapper-col="{ offset: 8, span: 16 }">
      <a-button :loading="submitLoading" type="primary" html-type="submit">登录</a-button>
    </a-form-item>
  </a-form>
</template>
<script lang="ts">
import { defineComponent, reactive,ref,getCurrentInstance } from 'vue';
import { LoginSendVertifyCode,Login } from '@/http/HttpAccountServer'
...
export default defineComponent({
  ...
  setup() {
    ...
    const onFinish = (values: any) => {
      ...
      localStorage.setItem("rememberme",formModel.remember?'1':'0')
      Login(formModel).then(val=>{
        console.log("登录结果",val)
        if(val.success){
          message.success('登录成功！')
          instance?.emit("loginres",10,"登录成功！")
          submitLoading.value=false;
        }else{
          submitLoading.value=false;
          notification.error({message:"登录失败！",description:val.message?val.message:'未返回错误信息',duration:10})
          instance?.emit("loginres",11,val.message)
        }
      }).catch(err=>{
        submitLoading.value=false;
        console.log("错误",err)
        notification.error({message:"登录请求发生错误！",description:err.message,duration:10})
        instance?.emit("loginres",11,err.message)
      })
    };
    ...
    const sendCode=()=>{
      formRef.value.validateFields('account').then(()=>{
        ...
        LoginSendVertifyCode(formModel).then(val=>{
          if(val.success){
